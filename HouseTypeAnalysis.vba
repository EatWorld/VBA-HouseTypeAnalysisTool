...
' 创建饼图数据区域（只包含非零数据）
Dim pieDataStartRow As Long
Dim pieDataEndRow As Long
Dim areaOrder As Variant
Dim areaCounts As Object

' 重新获取档位数组和计数对象（这些应该从主函数传递过来）
areaOrder = Array("50以下", "50-60", "60-70", "70-80", "80-100", "100-110", "110-120", "120-134", "135", "135以上")
Set areaCounts = CreateObject("Scripting.Dictionary")

' 从工作表中读取档位数据
For i = LBound(areaOrder) To UBound(areaOrder)
    areaCounts.Add areaOrder(i), ws.Cells(4 + i, 6).Value
Next i

pieDataStartRow = 25  ' 在更下方创建饼图数据

' 准备饼图数据标题
ws.Cells(pieDataStartRow, 20).Value = "档位"
ws.Cells(pieDataStartRow, 21).Value = "套数"

' 只复制非零数据到饼图数据区域
Dim pieRow As Long
pieRow = pieDataStartRow + 1
For i = LBound(areaOrder) To UBound(areaOrder)
    If areaCounts(areaOrder(i)) > 0 Then
        ws.Cells(pieRow, 20).Value = areaOrder(i)
        ws.Cells(pieRow, 21).Value = areaCounts(areaOrder(i))
        pieRow = pieRow + 1
    End If
Next i
pieDataEndRow = pieRow - 1

' 创建饼图 - 增大尺寸并调整位置
Set pieChartObj = ws.ChartObjects.Add(Left:=ws.Columns("I").Left, Top:=ws.Rows(17).Top, Width:=500, Height:=400)
...
With pieChartObj.Chart
    ...
    ' 使用过滤后的非零数据
    .SetSourceData Source:=ws.Range(ws.Cells(pieDataStartRow + 1, 21), ws.Cells(pieDataEndRow, 21))
    .SeriesCollection(1).XValues = ws.Range(ws.Cells(pieDataStartRow + 1, 20), ws.Cells(pieDataEndRow, 20))
    
    ' 显示图例
    .HasLegend = True
    With .Legend
        .Position = xlLegendPositionRight  ' 图例放在右侧
        .Font.Size = 10
        .Font.Bold = False
    End With
    ...
    ' 计算总套数用于百分比计算
    Dim j As Long
    Dim totalPieCount As Long
    totalPieCount = 0
    For j = pieDataStartRow + 1 To pieDataEndRow
        totalPieCount = totalPieCount + ws.Cells(j, 21).Value
    Next j
    
    ' 为每个数据点设置双重标签：内部显示档位名称，外部显示套数和百分比
    For j = 1 To .SeriesCollection(1).Points.Count
        Dim pointIndex As Long
        pointIndex = j
        pointValue = ws.Cells(pieDataStartRow + j, 21).Value
        Dim pointPercentage As Double
        If totalPieCount > 0 Then
            pointPercentage = Round((pointValue / totalPieCount) * 100, 1)
        Else
            pointPercentage = 0
        End If
        ...
        categoryName = ws.Cells(pieDataStartRow + j, 20).Value
        
        ' 设置点的数据标签
        With .SeriesCollection(1).Points(pointIndex)
            .HasDataLabel = True
            With .DataLabel
                .Text = categoryName  ' 内部显示档位名称
                .Position = xlLabelPositionCenter  ' 标签放在扇形中心（内部）
                .Font.Size = 12
                .Font.Bold = True
                .Font.Color = RGB(255, 255, 255)  ' 白色字体便于在色块上显示
            End With
            
            ' 添加第二个标签显示套数和百分比（外部）
            .ApplyDataLabels
            .DataLabel.Text = categoryName & vbCrLf & pointValue & "套," & pointPercentage & "%"
            .DataLabel.Position = xlLabelPositionBestFit
            .DataLabel.Font.Size = 10
            .DataLabel.Font.Bold = True
            .DataLabel.Font.Color = RGB(0, 0, 0)
        End With
    Next j
End With

' 隐藏饼图辅助数据
With ws.Range(ws.Cells(pieDataStartRow, 20), ws.Cells(pieDataEndRow, 21))
